# Lovable Update Prompt — Path Change Hub + Transition Modules
...
```
Add a new Path Change system: a persistent hub accessible from the main menu, plus 11 transition module screens covering every path-to-path switch. This is new content, not a rework of anything existing. Do not change the app shell, bottom nav destinations that already exist, home screen, chat, This Week, onboarding, or any Getting Started guide's content or route, except for the three small contextual-CTA additions specified in Part 5 below.

---

PART 0 — WHAT'S CHANGING (SUMMARY)

1. New route: /path-change — the hub. Reachable from the main menu at all times, regardless of the user's current path. Entry point language is neutral and non-shaming, not tucked away or hard to find.

2. New routes: /path-change/[transition-id] — 11 transition module screens (T-A-B, T-A-C, T-A-F, T-B-A, T-B-C, T-B-F, T-C-A, T-C-B, T-C-F, T-REL, T-WEAN). Each follows a shared template: LEAD LINE, WHAT TO KNOW, WHAT TO DO, WHAT TO WATCH FOR, expandable CRASH-COURSE SNIPPET. Same visual language as the Getting Started guide screens (same header treatment, same card/section styling), so this reads as part of the same app, not a bolted-on feature.

3. One module, T-A-B (Nursing to Exclusive Pumping), has a special behavior: it renders one of 4 content windows based on how many days postpartum the user is, computed from the baby's date of birth already captured at onboarding. See Part 6.

4. Three existing Getting Started guides get a small contextual addition: a "Thinking about changes?" link pointing into the hub. These are 1-2 sentence additions, not structural changes. See Part 5.

5. No migration needed for this pass. The actual "confirm my new path" action inside the hub should call a `switch-feeding-path` function once it exists on the backend (tracked separately, not part of this Lovable update). Until that function ships, wire the button to call it as a stub that currently no-ops or falls back to the existing profile-update path, and flag it clearly in code as temporary. Do not have this button write anything to analytics or tracking tables directly. That's backend-owned.

6. This system does not track or display to the user how many times they've visited the hub, which transitions they've viewed, or any switch history. No such UI exists anywhere in this feature.

---

PART 1 — THE PATH CHANGE HUB

ROUTE: /path-change

Nav entry: add "Path Change" (or similar short label matching the app's existing nav item naming convention) to the main menu, visible to every user regardless of current path. Do not gate this nav item by path; every user can reach every transition from every starting point (except formula, which only shows T-REL as its outbound option, and T-WEAN reads as a general reference rather than a specific pairing).

Hub screen structure, top to bottom:

  HEADER: "Thinking about changing how you feed?" (this exact copy, locked). One line of supporting text below it: "Whatever's bringing you here, we've got what you need. No judgment, no pressure to explain."

  MATRIX / SELECTOR: a simple two-step or single-screen picker: "I'm currently on [Nursing / Exclusive Pumping / Combination Feeding / Formula]" then "and I'm thinking about [the other paths, filtered to valid destinations from Part 2 below]." Selecting a valid pair routes to the matching transition module. If the user's current app path is already known (it is, from their profile), pre-select "I'm currently on" to match their existing path rather than making them re-select it, but still let them change it manually in case they're exploring on someone else's behalf or just browsing.

  Below the selector: a plain list of all 11 transition modules as tappable rows (title + one-line description, using the LEAD LINE-adjacent copy from Part 4 below), so a user can browse directly without using the selector if they already know what they're looking for. Include T-WEAN in this list as a standalone entry, not paired to a specific from/to combination.

  The hub does not show a "you are here" indicator, streak, or any gamified element. Keep this screen calm and low-pressure, visually quieter than the Getting Started library screen.

---

PART 2 — VALID TRANSITIONS (routes the selector must support)

  From Nursing (A): to Exclusive Pumping (T-A-B), to Combination Feeding (T-A-C), to Formula (T-A-F)
  From Exclusive Pumping (B): to Nursing (T-B-A), to Combination Feeding (T-B-C), to Formula (T-B-F)
  From Combination Feeding (C): to Nursing (T-C-A), to Exclusive Pumping (T-C-B), to Formula (T-C-F)
  From Formula: to any nursing path (T-REL) — this is the only outbound option from Formula, and it should read clearly as "bringing your milk back" rather than offering a specific A/B/C choice, since T-REL is intentionally a single thin module that routes to an IBCLC rather than 3 separate destination-specific modules.

  T-WEAN is not paired to a specific transition. It's a general weaning reference available from the hub's module list regardless of current or destination path.

---

PART 3 — TRANSITION MODULE SCREEN TEMPLATE (shared structure, all 11 modules)

Each module screen at /path-change/[id] renders, top to bottom:

  HEADER: bold title line (all caps per the source copy, e.g. "STOPPING NURSING. HERE'S HOW TO DO IT WELL."), then a small metadata line below it: "Path Change • [Origin] → [Destination] • Estimated read time: [X] min" (use the exact read-time value given per module in Part 4).

  LEAD LINE: rendered as a standalone, slightly larger/emphasized line directly under the header, same visual treatment as a pull-quote or the emphasized opening line pattern already used somewhere in the Getting Started guides if one exists; otherwise a simple larger/bolder single line of body text.

  WHAT TO KNOW: standard body copy section, h2-style label "What to know", paragraphs as written.

  WHAT TO DO: numbered list, h2-style label "What to do".

  WHAT TO WATCH FOR: standard body copy section, h2-style label "What to watch for". If the module contains a bolded standalone warning line (e.g. "Fever + breast pain + flu symptoms: call your OB today"), render that as a highlighted callout using the same warning-style treatment used elsewhere in the app for medical escalation callouts, not as plain paragraph text.

  CRASH-COURSE SNIPPET: expandable/collapsible section, collapsed by default, labeled with the exact expand-prompt text given per module (usually "Want to go deeper?", T-REL uses "Find an IBCLC"). Reuse whatever expand/collapse component pattern already exists in the Getting Started guides' crash-course sections.

  BOTTOM: a link back to the hub ("Back to Path Change") and, where the module content references it, a link into the relevant Path A/B/C or Getting Started content it points to (e.g. T-A-F points to formula prep content, T-B-A mentions IBCLC directory). Do not add a "mark complete" or progress-tracking control to these screens; transition modules are reference content, not sequenced modules, and should not affect Getting Started completion state.

---

PART 4 — FULL CONTENT, ALL 11 MODULES

Render each module's content exactly as written below. Voice notes: no em dashes in body prose (structural headers like the title lines are fine as-is), direct second-person address throughout.

====================
T-A-F — Nursing to Formula
====================

TITLE: STOPPING NURSING. HERE'S HOW TO DO IT WELL.
METADATA: Path Change • Nursing → Formula • Estimated read time: 4-5 min

LEAD LINE:
You don't owe anyone an explanation for this decision. Not here, not anywhere.

WHAT TO KNOW:
This module is about the how, what's going to happen in your body over the next 1 to 3 weeks, and what you can do to make this transition as comfortable as possible. We're skipping the why. You already know the why.

Weaning works better gradually. If your situation allows for it, if this isn't a sudden medical decision, your body will handle the transition more comfortably with a gradual reduction in nursing sessions rather than an abrupt stop. Abrupt cessation is possible, but it significantly increases the risk of engorgement leading to a blocked duct leading to mastitis. That's a miserable experience to add to an already hard transition. The recommendation: drop one nursing session every 2 to 3 days, not all at once.

Here's what that looks like in practice. If you're currently nursing 6 times per day, the gradual schedule might look like this: drop one session on day 1, stay at 5 sessions for 2 to 3 days, drop again, and so on. Your body will take several days between drops to adjust supply. You'll likely feel pressure and fullness after each drop, that's normal, and it passes. By the end of 2 to 3 weeks, most moms have completed the process and supply has wound down.
...
What your body is going to do. As nursing sessions decrease, your prolactin levels drop and your body gradually reabsorbs the milk it's producing. This process takes time, typically 1 to 3 weeks for supply to substantially decrease, and sometimes longer for traces of milk to be fully gone. You may notice: breast fullness and pressure, especially in the first few days after each dropped session; leaking, particularly with letdown triggers like hearing a baby cry, seeing photos of your baby, or warm showers; engorgement episodes if sessions are dropped too quickly.
...
WHAT TO DO:
1. Drop one session at a time, every 2 to 3 days. Start with the session your baby is least interested in, or the one that feels most optional in your current schedule. Let your body adjust before dropping the next one.
...
WHAT TO WATCH FOR:
During weaning, your risk of mastitis is elevated, specifically in the window when supply is high but nursing frequency is decreasing. Watch for: fever above 101°F; flu-like symptoms (body aches, chills, fatigue) combined with breast pain; a hard, red, or warm patch on the breast that doesn't resolve within 24 hours after the next session is dropped.

CALLOUT (render as warning-style highlighted callout): If you have fever + breast pain + flu symptoms: call your OB or provider today. Mastitis during weaning is treated the same way as mastitis during nursing, antibiotics, continue expressing to prevent progression. Do not wait it out.
...
CRASH-COURSE SNIPPET (expand label: "Want to go deeper?"):
Formula basics: what to know if you're new to it.

All infant formulas sold in the US must meet the same FDA nutritional standards. The primary distinction you'll encounter: cow's milk-based (the most common, appropriate for most babies unless there's a documented intolerance or allergy); soy-based (sometimes recommended for dairy intolerance, ask your pediatrician); hydrolyzed or hypoallergenic (for babies with confirmed cow's milk protein allergy, usually recommended by a pediatrician, not chosen independently); ready-to-feed vs. powder vs. concentrate (ready-to-feed is the most expensive and most convenient, powder is the most economical, concentrate is in between, all are nutritionally equivalent when prepared correctly).

Safe preparation for powder formula: wash hands; use water that's safe for your baby (tap water is fine in most US municipalities, if you're uncertain, ask your pediatrician); follow the package ratio exactly, too concentrated or too diluted is not safe; mix thoroughly, let cool before offering; use within 1 hour if left at room temperature, or within 24 hours if refrigerated; never microwave formula, it creates hot spots that can burn a baby's mouth.

How much to offer: a general starting point for newborns is 2-3 oz every 2-4 hours, adjusting to your baby's hunger cues. Your pediatrician will help you calibrate based on your baby's weight and growth. The tracker in the app can log formula feeds just as it logs nursing sessions.

What "paced bottle feeding" means: even if you've already introduced bottles, it's worth noting: feed your baby semi-upright with the bottle held more horizontally, allowing the baby to control the flow with pauses every few sucks. This prevents overfeeding and helps regulate the experience. A feed should take 15-20 minutes, not 3.

====================
T-A-B — Nursing to Exclusive Pumping
====================

TITLE: SWITCHING TO THE PUMP. WHAT'S DIFFERENT FROM HERE.
METADATA: Path Change • Nursing → Exclusive Pumping • Estimated read time: 3-4 min

See Part 6 below for this module's special DOB-based window logic. It has 4 possible LEAD LINE / WHAT TO KNOW variants (Window 1: Days 1-4, Window 2: Days 5-14, Window 3: Weeks 2-6, Window 4: 6 weeks+), and shared WHAT TO DO / WHAT TO WATCH FOR / CRASH-COURSE sections with inline per-window substitutions. Do not build this one as plain static content; it needs the conditional rendering described in Part 6.

====================
T-B-F — Exclusive Pumping to Formula
====================

TITLE: HANGING UP THE PUMP. EVERYTHING YOU NEED TO KNOW.
METADATA: Path Change • Exclusive Pumping → Formula • Estimated read time: 4-5 min
...
What happens to your body when you stop pumping. When pump sessions decrease, your prolactin levels drop and your body gradually reduces milk production in response. The process takes time, typically 1 to 3 weeks for supply to substantially wind down, depending on how established your supply is and how quickly you reduce sessions. The key risk in this window is the same as with nursing: engorgement leading to a blocked duct leading to mastitis. The protection is a gradual reduction, not an abrupt stop.

The gradual approach. Drop one pumping session every 2 to 3 days. Start with the session that produces the least milk, typically a mid-day session. Let your supply adjust before dropping the next one. You'll feel breast fullness after each drop, which should ease within 24 to 48 hours as your body recalibrates to the new demand level.

What to expect between sessions. As you reduce, you may experience: breast pressure and fullness (manageable, brief expression for relief is fine); leaking at the times your body "expects" a session; engorgement if you drop too quickly, the signal to slow down, not push through.

How long it takes. Most EP moms who wean gradually are through the active process in 2 to 3 weeks. Traces of milk can be expressible for months after weaning, this is normal and doesn't mean production hasn't stopped. The body is very thorough about keeping the option open.
...
WHAT TO DO:
1. Drop one session every 2 to 3 days, starting with the mid-day session. Your mid-day sessions typically produce less than morning sessions (prolactin is highest in the morning). Start there.
2. Express for relief, not emptying. When you feel pressure between sessions, hand express or pump for 1 to 3 minutes, just enough to bring the fullness down to a comfortable level. Fully emptying sends a supply signal; brief relief expression does not.
...
WHAT TO WATCH FOR:
The window of highest mastitis risk is the first 1 to 2 weeks of active reduction. Watch for: fever above 101°F; flu-like symptoms, body aches, chills, fatigue, combined with breast pain; a localized red, warm, hard area of the breast that doesn't resolve.

CALLOUT (render as warning-style highlighted callout): Fever + breast pain + flu symptoms = call your OB today. Mastitis during pump weaning is treated the same way as mastitis at any other time: antibiotics, continued emptying (yes, even during weaning, stopping abruptly during mastitis worsens it).

Also watch for: milk blebs (small white spots on the nipple), usually self-resolving, mention to your IBCLC if painful or persistent; vasospasm (color change in the nipple after sessions, often painful), Raynaud's-related, your OB can advise on management if this is an issue.
...
Preparation: wash hands; measure water first (use room temperature or warm tap water, safe in most US municipalities); add the correct number of scoops per the package instructions, don't round up or down, ratio matters; shake or stir until fully dissolved; if warming, set the bottle in a cup of warm water, never microwave (creates hot spots); check temperature, a few drops on your inner wrist should feel neutral, not warm.

Storage: prepared formula, use within 1 hour at room temperature, or refrigerate and use within 24 hours; open powder, close the lid, store in a cool dry place, use within 1 month; ready-to-feed (opened), use within 48 hours, refrigerated.

How much to offer (render as a table): Newborn to 4 weeks, 2-3 oz, 8-12 feeds per day. 1-2 months, 3-4 oz, 7-8 feeds per day. 2-4 months, 4-6 oz, 6-7 feeds per day. 4-6 months, 6-7 oz, 5-6 feeds per day. These are reference ranges. Follow your baby's hunger cues and your pediatrician's guidance over any table.

Paced bottle feeding, especially important now that the bottle is the only method: hold your baby semi-upright. Keep the bottle more horizontal than vertical. Let the baby control the feed, pause every 20-30 sucks to give them a moment to register satiety. A full feed should take 15-20 minutes. This prevents overfeeding and helps your baby regulate their own intake signals.

====================
T-C-F — Combination Feeding to Formula
====================

TITLE: FINISHING THE MILK CHAPTER.
METADATA: Path Change • Combination Feeding → Formula • Estimated read time: 3-4 min
...
WHAT TO WATCH FOR:
Same mastitis risk window as any other weaning: fever above 101°F, flu-like symptoms with breast pain, or a warm, hard, red patch that doesn't ease up.

CALLOUT (render as warning-style highlighted callout): If you notice fever plus breast pain plus flu symptoms, call your OB or provider that day. Don't wait it out.
...
TITLE: TRYING THE BREAST AGAIN. WHAT TO EXPECT.
METADATA: Path Change • Exclusive Pumping → Nursing • Estimated read time: 3-4 min
...
CRASH-COURSE SNIPPET (expand label: "Want to go deeper?"):
Getting your baby back to the breast: try skin-to-skin time daily, even without a feeding goal attached to it; offer the breast when baby is calm, not starving; if latch feels off, breast compression during the feed can help keep milk flowing and hold baby's interest; a nipple shield is sometimes a useful bridge tool for a baby used to a bottle, an IBCLC can tell you if it's right for your situation; give it more than one or two tries before deciding it's not working, most babies need repetition to relearn or learn this skill.

====================
T-A-C — Nursing to Combination Feeding
====================

TITLE: ADDING SUPPLEMENTATION. WHAT TO KNOW.
METADATA: Path Change • Nursing → Combination Feeding • Estimated read time: 3-4 min
...
CRASH-COURSE SNIPPET (expand label: "Want to go deeper?"):
Paced bottle feeding, the short version: hold your baby semi-upright, not flat on their back; keep the bottle horizontal enough that your baby has to work a little for the milk, not vertical where gravity does all the work; pause every 20 to 30 sucks, giving your baby a break to notice they're getting full; a full feed should take roughly 15 to 20 minutes, not 3.
...
TITLE: ADDING FORMULA TO YOUR EP ROUTINE.
METADATA: Path Change • Exclusive Pumping → Combination Feeding • Estimated read time: 3 min
...
TITLE: MOVING TOWARD NURSING ONLY.
METADATA: Path Change • Combination Feeding → Exclusive Nursing • Estimated read time: 3 min
...
CRASH-COURSE SNIPPET (expand label: "Want to go deeper?"):
Signs supply is meeting the increased demand: steady or increasing wet diaper counts; weight tracking on your pediatrician's expected curve; your baby seeming satisfied and settled after nursing sessions, without immediately rooting again.
...
TITLE: MOVING TOWARD PUMPING ONLY.
METADATA: Path Change • Combination Feeding → Exclusive Pumping • Estimated read time: 3-4 min
...
TITLE: WEANING, THE GENERAL PLAYBOOK.
METADATA: Path Change • Any path → Supply wind-down reference • Estimated read time: 4 min
...
WHAT TO WATCH FOR:
Your risk of mastitis is highest in the window where supply is still relatively high but removal frequency is dropping. Watch for: fever above 101°F; flu-like symptoms (body aches, chills, fatigue) along with breast pain; a hard, red, or warm patch that doesn't resolve within a day of the next dropped session.

CALLOUT (render as warning-style highlighted callout): If you notice fever plus breast pain plus flu symptoms, call your OB or provider the same day. Mastitis during weaning is treated the same as mastitis at any other time: antibiotics, and continued gentle expression, not full stopping.
...
CRASH-COURSE SNIPPET (expand label: "Want to go deeper?"):
If you need to wean quickly: lean harder on relief expression and cold compresses since you're skipping the gradual runway; watch mastitis signs even more closely during the first week; consider talking to your OB about lactation-suppression options if your situation calls for a faster timeline.

A rough weaning timeline for reference: days 1 to 3 after a session drop, fullness, possible leaking; days 4 to 7, noticeably less fullness as supply adjusts; weeks 2 to 3, most active weaning complete for most people; beyond that, occasional traces can persist for weeks or longer, which is normal.

====================
T-REL — Relactation
====================

TITLE: BRINGING YOUR MILK BACK.
METADATA: Path Change • Formula → Any Nursing Path (Relactation) • Estimated read time: 2-3 min
...
CRASH-COURSE SNIPPET (expand label: "Find an IBCLC"):
...
For T-REL specifically, do not render a "browse other transitions" prompt or an "I'm now on Path X" confirmation control at the bottom of this module, unlike the other 10. This module intentionally routes out to an IBCLC rather than back into a path selection, per the locked product decision to keep T-REL thin.

---

PART 5 — CONTEXTUAL CTAs IN EXISTING GETTING STARTED CONTENT

Add a short CTA to these three existing Getting Started modules, matching whatever inline link/CTA styling already exists elsewhere in those guides. These are the only existing-content changes in this update.

  Module covering "Feeding Relationship Check-In" content: add "Thinking about changes? The Path Change hub has everything you need." linking to /path-change.
  Module covering "Combination Feeding Transition, If Needed" content: add direct links to /path-change/T-B-C and /path-change/T-B-F.
  Module covering "Deciding Your Week 7-12 Path" content: add a direct link to /path-change/T-C-F.

If you can't identify the exact existing module/route these map to from context, flag it rather than guessing, since these are the only three existing routes this update is allowed to touch.

---

PART 6 — T-A-B WINDOW LOGIC (DOB-based content variant)

T-A-B is the one module in this set that changes its LEAD LINE and WHAT TO KNOW content based on how many days postpartum the user is when they open it. Compute this from the baby's date of birth already stored from onboarding, the same way "week X" displays elsewhere in the app already work.

  Window 1 (days 1-4, colostrum phase): render this LEAD LINE and WHAT TO KNOW —

  LEAD LINE: Your milk hasn't fully come in yet, and that actually changes what this transition looks like, in a way that's useful to know.

  WHAT TO KNOW: Right now your body is producing colostrum: the dense, concentrated early milk that all babies receive in the first days after birth. Mature milk, the higher-volume production your baby will rely on for the months ahead, typically transitions in somewhere between days 3 and 5, and sometimes a day or two later after a cesarean or a complicated delivery. What this means for your situation: there is no established supply yet to protect or preserve. You're not abandoning something you built. You're building it now, with the pump as your tool. That's a different starting point, and a simpler one.
  ...
  Window 2 (days 5-14, milk arriving, early transition): render this LEAD LINE and WHAT TO KNOW —

  LEAD LINE: Your milk is coming in, or it just arrived. You're in the most supply-responsive window of your entire postpartum period, and that matters for how this transition goes.

  WHAT TO KNOW: Days 5-14 represent one of the most hormonally active periods in milk supply development. Colostrum is transitioning to mature milk, the higher-volume production your baby will rely on for the months ahead. Your body is actively calibrating supply based on the demand signals it's receiving right now, and it hasn't settled into a stable set point yet. What this means practically: the pump sessions you put in right now land with a disproportionately large effect on your long-term supply. Your body is in a highly responsive, still-negotiating state. Consistency in these two weeks is the leverage.
  ...
  Window 3 (weeks 2-6, supply establishing, not yet stable, this is the default if DOB data is unavailable): render this LEAD LINE and WHAT TO KNOW —

  LEAD LINE: You're in the most common window for this transition, and you're not starting over. But supply isn't fully established yet, what you do in the next few weeks matters.

  WHAT TO KNOW: Weeks two through six are when many mothers run into the latch difficulties, nipple damage, or logistical realities that bring them to this switch. It's also the window when supply has been building, sometimes substantially, but hasn't yet reached the stable set point it will find around weeks 6-8. Exclusive pumping at this stage can absolutely establish and maintain a full supply. The condition on that is session frequency: right now, the pump needs to do real work to complete the build that nursing started.
  ...
  Window 4 (6 weeks+, established supply): render this LEAD LINE and WHAT TO KNOW —

  LEAD LINE: Your supply is established. The transition from here is about maintaining what you have, which is a different, more manageable problem than building it.

  WHAT TO KNOW: At six weeks and beyond, supply has typically found its set point, not rigidly fixed, but stable enough that it operates on a demand-and-response system rather than the intensive establishment mode of the early weeks. You've been nursing consistently, your body has calibrated to that demand, and what you're changing now is the delivery mechanism. The supply itself doesn't need to change; the question is whether the pump will deliver the same demand signal that nursing was.
  ...
  SHARED WHAT TO DO (all windows), with per-window substitutions:
  ...
  2. Build your pump schedule before you drop nursing. Decide on your target sessions per day (8 is the minimum to maintain supply; 10 is better in the first 2-4 weeks of the transition). Map them across a 24-hour period. Keep one session between 12am-5am.
     WINDOW 1 SUBSTITUTION for step 2: "Build your pump schedule as your primary feeding system, 8-12 sessions per 24 hours, spaced no more than 3 hours apart during the day."
  3. Drop nursing sessions gradually as you add pump sessions. Don't switch cold turkey if you can avoid it. Replace one nursing session with a pump session every 1-2 days. This manages engorgement and gives your body time to adapt to the new stimulation pattern.
     WINDOW 1: step 3 does not apply (no established nursing pattern to drop). Substitute the hand expression guidance from the Window 1 WHAT TO KNOW instead.
     WINDOW 2 NOTE: add "Watch for engorgement if milk volume is increasing while nursing frequency is decreasing at the same time," since nursing may have been sporadic or assisted and milk may still be arriving.
  ...
  SHARED WHAT TO WATCH FOR (all windows), with per-window substitutions:

  In the first 1-2 weeks of the transition: output should be comparable to what nursing was providing. If you're seeing significantly less than expected after a week of consistent pumping at full frequency, a flange refit and technique review with your IBCLC is worth doing before assuming supply is dropping. (This first bullet applies to Windows 3 and 4 only.)
    WINDOW 1 SUBSTITUTION: "Colostrum volumes of 1-7 mL per session are normal. If you are expressing nothing at all after multiple sessions with correct technique and flange fit, contact your IBCLC."
    WINDOW 2 SUBSTITUTION: "Output should be increasing over the course of week 2 as mature milk volume builds. If output appears to plateau or decline after day 10 despite consistent sessions, flange fit and session frequency are the first things to check."
  ...
  SHARED CRASH-COURSE SNIPPET (all windows, expand label "Want to go deeper?"):
  Flange sizing, how to get it right. What you need: a flexible measuring tape or ruler. Measure your nipple diameter at its widest point, in millimeters, the nipple itself, not the areola. A typical nipple is 13-20mm in diameter. Add 2-3mm to your measurement, that's your target flange tunnel diameter. Check the flange size options for your specific pump brand, they vary by manufacturer. Most pumps come with a 24mm or 27mm flange, which fits a minority of nipples correctly.

  Signs your flange size is wrong: nipple rubbing the sides of the tunnel (too small, size up); nipple barely moving, areola being pulled heavily into the tunnel (too big, size down); pain at the base of the nipple during pumping; lower output than expected despite good session frequency.

  Hands-on pumping technique: before starting, apply gentle warmth, do a brief breast massage (circular, gentle, not aggressive); during the session, every few minutes, use your fingertips to compress the outer breast tissue toward the nipple while the pump is running; at the end of the session, when flow slows, switch the pump to expression mode, then do one final hand expression to clear any remaining milk.
...
PART 7 — DO NOT CHANGE

- Any existing Getting Started guide's route, content, or path gating, other than the two CTA/link additions specified in Part 5.
- The app shell, bottom nav destinations that already exist, home screen, chat, This Week, or onboarding.
- AppContext.moduleProgress or any completion-tracking state. Transition modules do not participate in completion tracking.
- Do not wire the hub's path-confirmation action to write anything to a database table directly from the frontend. It should call a backend function (stubbed for now per Part 0, item 5) so that analytics/audit logic stays server-side.
```
