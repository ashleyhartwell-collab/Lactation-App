-- Migration 00013: Seed companion_triggers and companion_content.
--
-- Inserts all 25 Anticipatory Guidance (AG) and 27 Milestone Memory (ML) entries.
-- Uses ON CONFLICT DO UPDATE — safe to re-run.
--
-- HOLD items seeded with held = true (never queued to users until cleared):
--   AG-016 — supply concern tone — awaiting Maya IBCLC review
--   AG-023 — IGT / breast reduction — awaiting specialist IBCLC review
--
-- To clear a HOLD after review:
--   UPDATE companion_triggers SET held = false WHERE id = 'AG-016';
--   UPDATE companion_triggers SET held = false WHERE id = 'AG-023';
--
-- Path variant columns (in_app_message_path_b, in_app_message_path_c) seeded NULL.
-- Populate separately for ML-003, ML-008, ML-012, ML-019 before end-to-end QA.
